...
# ----------------------------------------------------------------------
# Explainability: per-person source-record trace-back
# ----------------------------------------------------------------------
st.subheader("Investigate a person")
st.caption("Every lead above traces back to a source record. Pick a person to see the underlying evidence.")

name_to_id = {G.nodes[n].get("name", str(n)): n for n in G.nodes()}
selected_name = st.selectbox("Person", sorted(name_to_id.keys()))
